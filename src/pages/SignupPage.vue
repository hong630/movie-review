    ...
    <section class="signup-wrap">
      <div class="signup-card">
        ...
        <template v-if="step === 1">
          <h2 class="signup-title">이메일을 입력해주세요</h2>
          ...
          <div class="signup-form">
            ...
            <input
                ...
                @input="onInputEmail"
                @keydown.enter="goToPasswordStep"
            />

            <p v-if="email && !isEmailValid" class="signup-error">
              올바른 이메일 형식으로 입력해주세요.
            </p>
        ...
        <template v-else-if="step === 2">
          <h2 class="signup-title">비밀번호를 입력해주세요</h2>
          <p class="signup-subtitle">
          ...
          <div class="signup-form">
            ...
            />
            <p v-if="passwordTouched && !isPasswordRuleValid" class="signup-error">
              비밀번호 규칙을 다시 확인해주세요.
            </p>

            <ul class="signup-rule-list">
              <li class="signup-rule-item" :class="{ valid: hasPasswordMinLength }">
                <CheckedIcon v-if="hasPasswordMinLength" class="signup-rule-icon"/>
                <UnCheckedIcon v-else class="signup-rule-icon"/>
                <span>8자 이상 20자 이하</span>
              </li>
              <li class="signup-rule-item" :class="{ valid: hasPasswordLetter }">
                <CheckedIcon v-if="hasPasswordLetter" class="signup-rule-icon"/>
                <UnCheckedIcon v-else class="signup-rule-icon"/>
                <span>영문 1자 이상 포함</span>
              </li>
              <li class="signup-rule-item" :class="{ valid: hasPasswordNumber }">
                <CheckedIcon v-if="hasPasswordNumber" class="signup-rule-icon"/>
                <UnCheckedIcon v-else class="signup-rule-icon"/>
                <span>숫자 1자 이상 포함</span>
              </li>
              <li class="signup-rule-item" :class="{ valid: hasPasswordSpecial }">
                <CheckedIcon v-if="hasPasswordSpecial" class="signup-rule-icon"/>
                <UnCheckedIcon v-else class="signup-rule-icon"/>
            ...
            <ul class="signup-rule-list signup-rule-list--confirm">
              <li class="signup-rule-item" :class="{ valid: isPasswordConfirmMatched }">
                <CheckedIcon v-if="isPasswordConfirmMatched" class="signup-rule-icon"/>
                <UnCheckedIcon v-else class="signup-rule-icon"/>
                <span>비밀번호 확인 일치</span>
              </li>
            </ul>

            <p v-if="passwordConfirmTouched && !isPasswordConfirmMatched" class="signup-error">
              비밀번호가 일치하지 않습니다.
            </p>
        ...
        <template v-else-if="step === 3">
          <h2 class="signup-title">이메일을 확인해주세요</h2>
          <p class="signup-subtitle">
            아래 이메일로 인증코드를 발송해주세요. <br/>
            인증 완료 후 회원가입을 진행할 수 있습니다.
          </p>
          ...
          <div class="signup-form">
            ...
            <div v-if="verificationSent" class="signup-verify-box">
              <label class="signup-label" for="verificationCode">인증코드</label>
              <input
                  id="verificationCode"
                  class="signup-input"
                  type="text"
              ...
              <div class="signup-timer-row">
                <span class="signup-timer">코드 유효시간 {{ formattedRemainSeconds }}</span>
                <span class="signup-resend">
                  {{
              ...
              <p v-if="verificationMessage && !verificationVerified" class="signup-info">
                {{ verificationMessage }}
              </p>

              <p v-if="verificationVerified" class="signup-success">
                이메일 인증이 완료되었습니다.
              </p>
        ...
        <!-- STEP 4: 완료 -->
        <template v-else>
          <h2 class="signup-title">회원가입이 완료되었습니다</h2>
          <p class="signup-subtitle">
            이제 로그인 후 영화 기록을 시작하실 수 있습니다.
          </p>

          <div class="signup-complete-mark">
            <HappyIcon/>
          </div>
...
class SignupPage extends Vue {
  ...
  get canGoToVerifyStep(): boolean {
    return this.isPasswordRuleValid && this.isPasswordConfirmMatched;
  }
  ...
  goToVerifyStep() {
    if (!this.canGoToVerifyStep) return;
    this.step = 3;
  }
  ...
  completeSignup() {
    if (!this.canCompleteSignup) return;

    this.password = '';
    this.passwordConfirm = '';
    this.verificationCode = '';
    this.step = 4;
    this.openToast('success', '회원가입이 완료되었습니다.', 3000);
  }
  ...
  mounted() {
    //this.openToast('info', '이 메시지는 직접 닫기 전까지 유지돼요.', null);
  }
