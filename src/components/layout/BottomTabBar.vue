<template>
  <nav v-if="showBottomTabBar" class="tabbar">
...
class BottomTabBar extends Vue {
  ...
  private readonly HIDE_BOTTOM_TAB_PATHS = [
    '/login',
    '/find-id',
    '/find-password',
    '/signup',
  ...
  get showBottomTabBar(): boolean {
    const path = router.currentRoute.value.path;
    return !this.HIDE_BOTTOM_TAB_PATHS.includes(path);
  }
